...
abstract class TechnicianCredentialRepository {
  Future<String> createTechnicianUser(TechnicianCredential technicianCredential);
}

class TechnicianCredentialRepositoryimpl implements TechnicianCredentialRepository{
  @override
  Future<String> createTechnicianUser(TechnicianCredential technicianCredential) async {
    final response = await http.post(
      Uri.parse('http://10.0.2.2:3000/technician/signup'),
      headers: {'Content-Type': 'application/json'},
      body: json.encode(technicianCredential.toJson()) // Convert the user object to JSON
    );
    // print(response.body);
