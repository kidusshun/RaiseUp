...
abstract class CustomerConfirmationCredentialRepository {
  Future<String> confirmCustomerUser(
      CustomerConfirmationCredential customerConfirmationCredential);
}

class CustomerConfirmationCredentialRepositoryimpl
    implements CustomerConfirmationCredentialRepository {
  final secureStorage = new FlutterSecureStorage();
  @override
  Future<String> confirmCustomerUser(
      CustomerConfirmationCredential customerConfirmationCredential) async {
    final response = await http.post(
        Uri.parse('http://10.0.2.2:3000/customer/signin'),
        headers: {'Content-Type': 'application/json'},
        body: json.encode(customerConfirmationCredential
            .toJson()) // Convert the user object to JSON
        );
